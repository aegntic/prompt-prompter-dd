...
# Silent Assembly - Final Revision (Strict Timing + No Stills)
# Total Duration: 168s (2:48)

mkdir -p output
STILLS="../demo_stills"
REC="rec"

echo "=== Generating Intermediate Clips for Static Images ==="

# Generate Shot 40 (Hands) - 2s Zoom
ffmpeg -y -loop 1 -i "$STILLS/11.png" \
  -vf "zoompan=z='min(zoom+0.0015,1.5)':d=60:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)',scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1" \
  -c:v libx264 -t 2 -pix_fmt yuv420p output/shot_40_hands.mp4

# Generate Shot 41 (End Card) - 2s Static (or slight zoom)
ffmpeg -y -loop 1 -i "$STILLS/12.png" \
  -vf "zoompan=z='min(zoom+0.0015,1.5)':d=60:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)',scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1" \
  -c:v libx264 -t 2 -pix_fmt yuv420p output/shot_41_end.mp4
...
ffmpeg -y \
  ...
  -t 4 -i "$STILLS/10.mp4" \
  -t 4 -i "$STILLS/03.mp4" \
  -i "output/shot_40_hands.mp4" \
  -i "output/shot_41_end.mp4" \
  -filter_complex " \
    ...
    [19:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1,fps=30[v19]; \
    [20:v]scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2,setsar=1,fps=30[v20]; 
